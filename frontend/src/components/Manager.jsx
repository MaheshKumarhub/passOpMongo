...
const Manager = () => {
  ...
  const savePassword = async () => {
   ...
   const deletePassword = async (id) => {
    try {
      const c = window.confirm("Do you really want to delete this password?");
      if (c) {
        await fetch(`https://passop-backend.onrender.com?id=${id}`, {
          method: "DELETE",
      ...
      toast('Error: Failed to delete password!', { type: 'error' });
    }
  };
                ...
                {passwordArray.map((item, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td className="py-2 border border-white text-center ">
                        <div className="flex items-center justify-center">
                        {item.password && <span>{'*'.repeat (item.password.length)}</span>}
                      ...
                      <td className=" justify-center py-2 border border-white text-center ">
                        <span className="cursor-pointer mx-2" onClick={()=>{editPassword(item.id)}}><FontAwesomeIcon icon={faPenToSquare} /></span>
                        <span className="cursor-pointer mx-2"  onClick={()=>{deletePassword(item.id)}}><FontAwesomeIcon icon={faTrash} /></span>
                      </td>
                    </tr>
                  );
                })}
